...
/**
 * ID/EX Pipeline Register: buffers decoded instruction data for execution stage
 *
 * Transfers decoded control signals and operand data from Instruction Decode
 * to Execute stage. This is the widest pipeline register, carrying all
 * information needed for instruction execution.
 *
 * Data Categories:
 *
 * **Instruction Information:**
 * - Original instruction (for debugging/tracing)
 * - Instruction address (PC value)
 *
 * **Register Operands:**
 * - Register addresses (rs1, rs2, rd)
 * - Register data values (fetched from register file)
 * - Write control (enable, source selection)
 *
 * **ALU Control:**
 * - ALU operand sources (register vs immediate vs PC)
 ...
 *
 * Control Signals:
 * - `flush`: Insert NOP on control hazards or load-use stalls
 *   - Prevents wrong operations from executing
 *   - Critical for maintaining correctness
 *
 * Hazard Handling:
 * - Flush converts all control signals to safe defaults
 * - Write enables → disabled
 * - Memory operations → cancelled
 * - Effectively inserts a bubble in the pipeline
 *
 * @note No stall signal - this register only supports flush
 * @note Flush is essential for inserting bubbles during hazards
 */
