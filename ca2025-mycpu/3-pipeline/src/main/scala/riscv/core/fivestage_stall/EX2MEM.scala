...
/**
 * EX/MEM Pipeline Register: buffers execution results for memory stage
 *
 * Transfers ALU computation results and memory operation data from Execute
 * to Memory Access stage. Critical for forwarding as it holds the most
 * recent computation results.
 *
 * Data Stored:
 *
 * **Execution Results:**
 * - ALU result (computation output or memory address)
 * - CSR read data (for CSR instructions)
 *
 * **Memory Operation Data:**
 * - Memory control signals (read/write enables)
 * - Store data (reg2_data for store instructions)
 * - Function code (funct3) for access width (byte/halfword/word)
 *
 * **Register Write Information:**
 * - Write enable and destination address
 * - Write source selector (ALU/memory/CSR)
 * - Used by forwarding unit for hazard detection
 *
 * Forwarding Role:
 * - Provides ForwardFromMEM path (priority 1)
 * - ALU result can be forwarded immediately
 * - Load results not available until next cycle
 *
 * Pipeline Flow:
 * ```
 * EX Stage → [EX2MEM Register] → MEM Stage
 *            ↓
 *      Forwarding to EX/ID stages
 * ```
 *
 * @note No control signals - always transfers on clock edge
 * @note Critical for data forwarding from most recent instruction
 * @note Load-use hazards detected using memory_read_enable signal
 */
