...
class RegisterFile extends Module {
  ...
  // Priority: x0 check → write forwarding → register read (with address mapping)
  io.read_data1 := MuxCase(
    registers(io.read_address1 - 1.U), // Default: read from physical storage (mapped)
    IndexedSeq(
      (io.read_address1 === 0.U)                                 -> 0.U,          // x0 always zero
      (io.read_address1 === io.write_address && io.write_enable) -> io.write_data // Forward write data
    )
  )

  io.read_data2 := MuxCase(
    registers(io.read_address2 - 1.U), // Default: read from physical storage (mapped)
    IndexedSeq(
      (io.read_address2 === 0.U)                                 -> 0.U,          // x0 always zero
      (io.read_address2 === io.write_address && io.write_enable) -> io.write_data // Forward write data
    )
  )

  io.debug_read_data := MuxCase(
    registers(io.debug_read_address - 1.U), // Default: read from physical storage (mapped)
    IndexedSeq(
      (io.debug_read_address === 0.U)                                 -> 0.U,          // x0 always zero
      (io.debug_read_address === io.write_address && io.write_enable) -> io.write_data // Forward write data
    )
  )
